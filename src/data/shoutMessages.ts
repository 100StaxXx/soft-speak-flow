// Premade shout messages for guild rivalry system
...
export const SHOUT_MESSAGES: ShoutMessage[] = [
  // 🔥 HYPE (Encouraging)
...
export const getShoutsByType = (type: ShoutType): ShoutMessage[] => {
  return SHOUT_MESSAGES.filter(msg => msg.type === type);
};

export const getShoutByKey = (key: string): ShoutMessage | undefined => {
  return SHOUT_MESSAGES.find(msg => msg.key === key);
};

export const SHOUT_TYPE_CONFIG: Record<ShoutType, { label: string; emoji: string; color: string }> = {
  hype: { label: 'Hype', emoji: '🔥', color: 'text-orange-500' },
  challenge: { label: 'Challenge', emoji: '⚔️', color: 'text-red-500' },
  support: { label: 'Support', emoji: '💜', color: 'text-purple-500' },
  taunt: { label: 'Taunt', emoji: '😈', color: 'text-emerald-500' },
};
