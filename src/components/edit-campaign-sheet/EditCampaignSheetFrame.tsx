...
export type EditCampaignSheetEpic = {
  id: string;
  title: string;
  description?: string | null;
  target_days: number;
...
export type EditCampaignSheetDependencies = {
...
export interface EditCampaignSheetFrameProps {
...
function CampaignRitualDifficultySelector({
  value,
  onChange,
}: {
  value: QuestFormDifficulty;
  onChange: (value: QuestFormDifficulty) => void;
}) {
  return (
    <div className="space-y-3">
      ...
      <RadioGroup value={value} onValueChange={onChange} className="grid grid-cols-3 gap-3">
        {ritualDifficultyOptions.map((option) => {
          const Icon = option.icon;
          const isSelected = value === option.value;

          return (
            <div key={option.value} className="relative">
              <RadioGroupItem value={option.value} id={`campaign-ritual-${option.value}`} className="peer sr-only" />
              <Label
...
export function EditCampaignSheetFrame({
  epic,
  open,
  onOpenChange,
  onDeleted,
              ...
              <section className={cn(plannerPathfinderTheme.raisedPanel, "space-y-4 p-4")}>
                ...
                {showAddRitual ? (
                  <div className={cn(plannerPathfinderTheme.mutedPanel, "space-y-4 border-dashed p-4")}>
                    ...
                    <CampaignRitualDifficultySelector
