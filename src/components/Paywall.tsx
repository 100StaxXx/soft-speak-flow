...
const PaywallLandscapeSection = ({
  ...
  >
    <div className="absolute inset-0 -z-10 bg-[linear-gradient(180deg,rgba(24,48,31,0.9)_0%,rgba(36,72,45,0.76)_40%,rgba(20,41,27,0.96)_100%),linear-gradient(90deg,rgba(25,50,32,0.9)_0%,rgba(47,89,56,0.35)_54%,rgba(25,50,32,0.82)_100%)]" />
...
export const getStoreKitFreeTrialLabel = (product: StoreKitProduct | null | undefined): string | null => {
  const offer = product?.introductoryOffer;
  if (!offer || offer.price !== 0 || !offer.paymentMode.toLowerCase().includes("free")) {
    return null;
  }

  const value = offer.periodValue;
  ...
  switch (offer.periodUnit) {
    ...
    case 1:
      return value === 1 ? "7-day free trial" : `${value}-week free trial`;
    case 2:
...
export const Paywall = ({ variant = "pre_trial_signup" }: PaywallProps) => {
  ...
  const yearlyProduct = useMemo(
    () => getProductForPlan("yearly", products, hasOfferCode),
    [hasOfferCode, products],
  );
  const selectedProductId = useMemo(
    () => getPurchaseProductIdForPlan(selectedPlan, products, hasOfferCode),
    [hasOfferCode, selectedPlan, products],
  );
  ...
  const selectedPlanRenewalText = hasSelectedCreatorYearlyOffer
    ? `${selectedPlanPrice}/year (${selectedPlanUnitPrice}); renews yearly at the same founding rate while the subscription remains active.`
  ...
  const freeTrialLabel = hasSelectedCreatorYearlyOffer
    ? null
    : getStoreKitFreeTrialLabel(selectedProduct);
  const hasSelectedFreeTrial = Boolean(freeTrialLabel);
          ...
          return (
            ...
            >
              <div className={cn("w-full", index === 0 ? "max-w-3xl" : "max-w-2xl")}>
                <div className="mb-5 inline-flex items-center gap-2 border border-white/[0.18] bg-black/[0.24] px-3 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-cyan-100 backdrop-blur-md">
                ...
                <p className="mt-6 max-w-2xl text-lg leading-7 text-white/[0.82] sm:text-2xl sm:leading-8">
                  {section.body}
                </p>
                {index === 0 ? (
                  <div className="mt-9 flex flex-col gap-3 sm:flex-row">
                    ...
                    <button
                      ...
                      className="inline-flex h-14 items-center justify-center border border-white/[0.18] bg-black/20 px-5 text-xs font-semibold uppercase tracking-[0.18em] text-white/[0.72] backdrop-blur-md transition hover:border-white/[0.36] hover:bg-white/10 hover:text-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-100 disabled:pointer-events-none disabled:opacity-50"
                    >
                      {copy.heroBadge}
                    </button>
                  </div>
        ...
        >
          <div className="grid w-full gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(360px,440px)] lg:items-center">
            <div className="space-y-6">
              <div>
                <div className="mb-4 inline-flex items-center gap-2 border border-white/[0.18] bg-black/[0.24] px-3 py-2 text-xs font-semibold uppercase tracking-[0.24em] text-cyan-100 backdrop-blur-md">
                ...
                <p className="mt-5 max-w-2xl text-lg leading-7 text-white/[0.82] sm:text-xl sm:leading-8">
                  {copy.subtitle}
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                {paywallBenefits.map((benefit) => {
                  ...
                  return (
                    <div
                      key={benefit.title}
                      className="border border-white/[0.14] bg-black/[0.24] p-4 backdrop-blur-md"
                    >
                      <div className="flex items-center gap-3">
                        <span className="flex h-9 w-9 items-center justify-center border border-cyan-100/[0.22] bg-cyan-100/10 text-cyan-100">
                          <Icon className="h-4 w-4" />
                        </span>
            ...
            <div className="border border-white/[0.16] bg-black/[0.42] p-4 shadow-[0_28px_80px_rgba(0,0,0,0.38)] backdrop-blur-2xl sm:p-5">
              <div className="space-y-5">
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex h-12 w-12 items-center justify-center border border-white/[0.16] bg-white/10">
                    <Trophy className="h-5 w-5 text-stardust-gold" />
                  </div>
                </div>

                {variant === "pre_trial_signup" && hasSelectedFreeTrial ? (
                  <div
                    data-testid="paywall-trial-callout"
                    className="border border-cyan-100/[0.36] bg-cyan-100/[0.12] p-4 shadow-[0_0_34px_rgba(165,243,252,0.12)]"
                  >
                ...
                <div className="space-y-3 border border-white/[0.12] bg-white/[0.04] p-4">
                  <div className="flex items-start gap-3">
                    <div className="mt-0.5 bg-cyan-100/[0.12] p-2 text-cyan-100">
                      <Gift className="h-5 w-5" />
                    </div>
                    <div className="space-y-1">
                      <h4 className="text-base font-semibold text-white">Have a founding or referral code?</h4>
                      <p className="text-sm text-white/[0.68]">
                        Enter it here before checkout to unlock an eligible founding rate.
                      </p>
                    </div>
                  </div>
                  ...
                  {hasAppliedReferralCode ? (
                      ...
                      <div className="flex items-start gap-3">
                        ...
                        <div className="space-y-1">
                          <p className="font-medium text-white">
                            {hasOfferCode ? "Founding rate unlocked" : "Referral code applied"}
                          </p>
                          <p className="text-sm text-white/70">
                            {hasOfferCode
                              ? `Your annual plan renews at ${activeYearlyOfferPrice} while it remains active.`
                    ...
                    <form className="space-y-3" onSubmit={handleApplyOfferCode}>
                      <Input
                        ...
                        maxLength={256}
                        className="h-12 border-white/15 bg-black/40 text-center text-base uppercase tracking-[0.2em] text-white placeholder:text-white/[0.36]"
                      />
                      <Button
                        type="submit"
                      ...
                      <p className="text-center text-xs text-white/[0.54]">
                        Eligible founding codes unlock the $29.99 annual Apple subscription.
                      </p>
                    </form>
                  )}
                </div>
                  ...
                  {(["monthly", "yearly"] as PlanType[]).map((plan) => (
                    <button
                      ...
                      className={cn(
                        ...
                        selectedPlan === plan
                          ? "border-cyan-100 bg-cyan-100/[0.12] shadow-[0_0_34px_rgba(165,243,252,0.16)]"
                          : "border-white/[0.14] bg-white/[0.04] hover:border-white/[0.32]",
                      )}
                    >
                      {plans[plan].savings && (
                      ...
                      <p className="text-xs font-semibold uppercase tracking-[0.2em] text-white/[0.58]">
                        {plans[plan].title}
                      </p>
                      ...
                      <p className="text-xs text-white/[0.62]">
                ...
                <div className="border border-white/[0.12] bg-white/[0.04] p-3 text-xs leading-relaxed text-white/[0.64]">
                ...
                <Button
                  asChild
                  variant="outline"
                  className="w-full border-cyan-100/34 bg-cyan-100/10 text-cyan-50 hover:bg-cyan-100/[0.18] hover:text-white"
                >
                  <a
                ...
                {productsLoading && (
                  <div className="flex items-center gap-2 border border-dashed border-white/[0.22] px-3 py-2 text-sm text-white/[0.64]">
                ...
                <Button
                  ...
                  className="w-full text-white/[0.64] hover:bg-white/[0.08] hover:text-white"
                >
                  <RefreshCw className="mr-2 h-4 w-4" />
                  Restore Purchases
                </Button>

                <div className="space-y-2 border-t border-white/[0.12] pt-4">
                  <p className="text-center text-xs leading-relaxed text-white/[0.54]">
                    {PREMIUM_PLAN_NOTE}
                  </p>
                  <p className="text-center text-xs leading-relaxed text-white/[0.54]">
                    {copy.legalIntro}
                  ...
                  <div className="flex justify-center gap-4 text-xs">
                    <a
                      ...
                      className="text-white/[0.58] underline hover:text-white"
                    >
                ...
                <div className="space-y-2 text-center">
                  <p className="text-xs text-white/[0.46]">Need another account?</p>
                  <div className="flex items-center justify-center gap-3">
                    <Button
                      ...
                      className="h-auto p-0 text-xs text-white/[0.58] hover:text-white"
                    >
                      {isSigningOut ? "Signing out..." : "Sign out"}
                    </Button>
                    ...
                    <Button
                      ...
                      className="h-auto p-0 text-xs text-white/[0.58] hover:text-destructive"
                    >
                      Delete account
                    </Button>
                  </div>
            ...
            <AlertDialogDescription asChild>
              <div className="space-y-3 text-sm text-muted-foreground">
                <p>
                  ...
                  including your reflections, completed practices, saved progress, and account settings will be
                  permanently deleted.
                </p>
