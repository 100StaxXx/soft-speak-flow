...
import { getProfilesByIds } from "@/lib/firebase/profiles";
...
export const GuildMembersSection = ({ epicId }: GuildMembersSectionProps) => {
  ...
  useEffect(() => {
    const fetchMembers = async () => {
      ...
      try {
        ...
        // Batch fetch profiles and companions in parallel (much faster!)
        const [companionsMap, profilesMap] = await Promise.all([
          getCompanionsByUserIds(userIds),
          getProfilesByIds(userIds),
        ]);

        // Extract only needed profile fields
        const profileDataMap = new Map();
        profilesMap.forEach((profile, userId) => {
          profileDataMap.set(userId, {
            email: profile.email,
  ...
  const getDisplayName = useCallback((member: LeaderboardMember) => {
    return getUserDisplayName(member.profile);
  }, []);

  const handleOpenShoutDrawer = useCallback((member: LeaderboardMember) => {
    setSelectedMember(member);
    setShoutDrawerOpen(true);
  }, []);

  const handleSendShout = useCallback((shoutType: ShoutType, messageKey: string) => {
  ...
  }, [selectedMember, sendShout]);

  const handleSetRival = useCallback((memberId: string) => {
        ...
        <CardContent className="space-y-2">
          {members.map((member, index) => (
            <GuildMemberItem
              ...
              member={member}
              index={index}
              isCurrentUser={member.user_id === user?.uid}
              isRival={rivalry?.rival_id === member.user_id}
              isSettingRival={isSettingRival}
              getDisplayName={getDisplayName}
              getRankIcon={getRankIcon}
              onOpenShoutDrawer={handleOpenShoutDrawer}
...
const GuildMemberItem = memo(({
  ...
  isRival: boolean;
  isSettingRival: boolean;
  getDisplayName: (member: LeaderboardMember) => string;
  getRankIcon: (index: number) => React.ReactNode;
  onOpenShoutDrawer: (member: LeaderboardMember) => void;
  onSetRival: (memberId: string) => void;
}) => {
      ...
      {!isCurrentUser && (
        <div className="flex gap-1">
          <Button
            ...
            size="icon"
            className="h-8 w-8"
            onClick={() => onOpenShoutDrawer(member)}
          >
          ...
          {!isRival && (
            <Button
              ...
              onClick={() => onSetRival(member.user_id)}
...
GuildMemberItem.displayName = 'GuildMemberItem';
