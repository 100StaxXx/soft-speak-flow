...
import { WELLBEING_CATEGORIES } from "@/shared/companionWellbeing";
...
export function CompanionVideoPreparation() {
  ...
  useQuery({
    queryKey: ["companion-video-preparation", user?.id, companion?.id, stage, sourceImageUrl],
    enabled: Boolean(user && companion && stage > 0 && sourceImageUrl && !isLoading && accessState.has_access),
    queryFn: async () => {
      const results = await Promise.allSettled(WELLBEING_CATEGORIES.map((category) =>
