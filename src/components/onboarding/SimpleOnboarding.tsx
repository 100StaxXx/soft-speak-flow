...
import { findBestMentor, type OnboardingAnswer } from "@/utils/mentorMatching";
import { getZodiacSign, type ZodiacSign } from "@/utils/zodiacCalculator";
...
export const SimpleOnboarding = () => {
  ...
  const handleBirthdateComplete = async (date: Date) => {
  ...
  const handleQuestionnaireComplete = async (questionAnswers: OnboardingAnswer[]) => {
    ...
    // Find best mentor using the mentorMatching utility
    const bestMatch = findBestMentor(questionAnswers, mentors);

    if (bestMatch) {
      setRecommendedMentor(bestMatch as Mentor);

      // Convert answers to Record format for explanation generator
      const selectedAnswers: Record<string, string> = {};
      questionAnswers.forEach(answer => {
        selectedAnswers[answer.questionId] = answer.mentorTags[0] || "";
      });

      // Generate explanation
      const explanation = generateMentorExplanation(bestMatch as Mentor, selectedAnswers);
