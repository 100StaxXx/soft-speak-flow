...
export function Pathfinder({
  ...
  const [epicTitle, setEpicTitle] = useState(initialGoal);
  ...
  const {
    ...
    onFinalResult: (text) => {
      setEpicWhy((prev) => (prev ? prev + ' ' + text : text).trim());
      setInterimText('');
      success();
    },
  ...
  const handleGenerateTimeline = useCallback(async () => {
    if (!epicTitle.trim() || !deadline) return;
    tap();
    
    const deadlineStr = format(deadline, 'yyyy-MM-dd');
    // Combine title and "why" for a richer goal context
    const goalContext = epicWhy ? `${epicTitle}. Why: ${epicWhy}` : epicTitle;
    ...
    setStep('timeline');
    success();
  }, [epicTitle, epicWhy, deadline, clarificationAnswers, epicContext, timelineContext, generateSchedule, tap, success]);
  ...
  }, [schedule, deadline, epicTitle, epicWhy, adjustSchedule]);
  ...
  const handleProceedToStory = useCallback(() => {
    if (!epicWhy.trim() || !epicTitle.trim() || !deadline) return;
    // Use the "why" as the description if not already set
    if (!epicDescription) {
      setEpicDescription(epicWhy);
    }
    setStep('story');
    medium();
  }, [epicWhy, epicTitle, deadline, epicDescription, medium]);
  ...
  const handleCreateEpic = useCallback(() => {
    ...
    onCreateEpic({
      title: epicTitle,
      description: epicDescription || epicWhy || undefined,
    ...
    success();
  }, [selectedHabits, epicTitle, epicWhy, epicDescription, targetDays, storyType, themeColor, schedule, onCreateEpic, success, trackInteraction, suggestions]);
        ...
        <div ref={scrollContainerRef} className="flex-1 min-h-0 overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch', touchAction: 'pan-y' }}>
          <AnimatePresence mode="wait">
            ...
            {step === 'goal' && (
              ...
              >
                {/* Why - Purpose/Motivation (required first) */}
                <div className="space-y-2">
                  <Label htmlFor="why-input" className="text-base font-semibold">
                    Why are you embarking on this campaign?
                  </Label>
                  <p className="text-sm text-muted-foreground">
                    What's driving you? What will achieving this mean to you?
                  </p>
                  <div className="relative">
                    <Textarea
                      id="why-input"
                      placeholder="e.g., I want to provide a better future for my family, I want to prove to myself I can do hard things..."
                      value={isRecording ? epicWhy + (interimText ? ' ' + interimText : '') : epicWhy}
                ...
                {/* Campaign Name - only shows after "why" is filled */}
                <AnimatePresence>
                  {epicWhy.trim().length > 0 && (
                    ...
                    >
                      <div className="space-y-2">
                        ...
                        <Input
                          id="title-input"
                          placeholder="e.g., Operation Bar Exam, Marathon Mastery, Spanish Journey"
                          value={epicTitle}
                      ...
                      <div className="space-y-2">
                        ...
                        <Textarea
                          id="context-input"
                          placeholder="e.g., I already have some experience, I can only work on this on weekends..."
                      ...
                      <Button
                        ...
                        disabled={!epicWhy.trim() || !epicTitle.trim() || !deadline}
                  ...
                  <div className="space-y-4 pb-4">
                    ...
                    <div className="space-y-3 p-4 bg-muted/50 rounded-xl border">
                      <div>
                        ...
                        <p className="font-semibold text-lg">{epicTitle}</p>
                      </div>
                      {epicWhy && (
                        <div>
                          <Label className="text-xs text-muted-foreground">Your Why</Label>
                          <p className="text-sm text-muted-foreground">{epicWhy}</p>
                        </div>
                      )}
                    </div>

                    <div className="space-y-2">
                      <Label htmlFor="epic-desc">Additional Notes (optional)</Label>
                      <Textarea
                        id="epic-desc"
                        value={epicDescription !== epicWhy ? epicDescription : ''}
                        onChange={(e) => setEpicDescription(e.target.value)}
                        placeholder="Any additional details about your campaign..."
                        rows={2}
                      />
                    </div>
