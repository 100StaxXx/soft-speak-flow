...
describe("JourneysCompanionPlannerModal", () => {
  ...
  it("ignores mobile drawer close requests after the user types an unsent draft", () => {
    const onOpenChange = vi.fn();
    ...
    fireEvent.change(
      screen.getByTestId("journeys-companion-planner-text-input"),
      {
        target: { value: "Just vibing. How are you?" },
      },
    );
  ...
  it("ignores mobile drawer close requests fired by the send tap after the draft clears", () => {
    const onOpenChange = vi.fn();
    mocks.state.draftInput = "Just vibing. How are you?";
    ...
    const drawerProps = mocks.drawerRootProps
      .slice()
      .reverse()
