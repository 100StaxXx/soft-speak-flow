      ...
      <div className="flex min-h-0 flex-1 flex-col bg-[linear-gradient(180deg,rgba(255,248,231,0.06),rgba(255,248,231,0.02))]">
        <ScrollArea className="flex-1">
          <div className="space-y-3 p-4 sm:p-5" data-testid="journeys-companion-planner-transcript">
            ...
            {planDay ? (
              <div className="rounded-[1.5rem] border border-white/12 bg-white/6 p-4 text-white">
                  ...
                  <div className="mt-4 space-y-3">
                    {planDay.suggestedQuests.map((quest) => (
                      <button
                        ...
                        className="w-full rounded-[1.25rem] border border-white/12 bg-black/10 p-4 text-left transition hover:border-white/20 hover:bg-white/8 disabled:cursor-not-allowed disabled:opacity-70"
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="rounded-full border border-white/12 px-2 py-1 text-[11px] uppercase tracking-[0.18em] text-white/65">
                            {quest.type}
                          </span>
                        </div>
            ...
            {comingUp ? (
              <div className="rounded-[1.5rem] border border-white/12 bg-white/6 p-4 text-white">
                ...
                {comingUp.nextEvent ? (
                  <div className="mt-3 rounded-[1.1rem] border border-white/10 bg-black/10 p-3">
                ...
                {comingUp.missedItems.length > 0 ? (
                  <div className="mt-3 space-y-2">
                    {comingUp.missedItems.map((item) => (
                      <div
                        key={item.id}
                        className="rounded-[1rem] border border-amber-300/15 bg-amber-400/10 px-3 py-2"
                      >
                        <p className="text-sm font-medium text-white">{item.title}</p>
                        <p className="text-sm text-white/65">{item.label}</p>
                      </div>
