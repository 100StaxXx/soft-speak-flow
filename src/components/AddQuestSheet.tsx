...
import { Plus, Sliders, CalendarIcon, Inbox, Map, X, Zap, Flame, Mountain, Clock, ChevronRight, Trash2 } from "lucide-react";
      ...
      <SheetContent side="bottom" className="h-[92vh] rounded-t-2xl flex flex-col p-0 gap-0 overflow-hidden">
        ...
        <div className={cn("relative px-5 pt-4 pb-5 flex-shrink-0", colors.bg)}>
          ...
          <div className="flex flex-col items-center text-center pt-2 text-white">
            ...
            <p className="text-sm opacity-80 mt-1.5">{summaryLine}</p>
            <Input
              ref={inputRef}
          ...
          <div className="px-4 py-4 space-y-3">
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  if (!scheduledTime) {
                    // Default to next rounded 15-min
                    const now = new Date();
            ...
            {/* Time Wheel (shown when time chip is active) */}
            {showTimePicker && (
              <div
                ref={timeWheelRef}
