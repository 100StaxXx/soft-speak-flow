...
import type { InboxQuest } from "@/features/quests/editing";
...
export type InboxQuestItem = InboxQuest;

interface QuestInboxSectionProps {
  quests: InboxQuestItem[];
  isLoading: boolean;
  isExpanded: boolean;
  ...
  onEditQuest: (quest: InboxQuestItem) => void | Promise<void>;
...
export const QuestInboxSection = memo(function QuestInboxSection({
  ...
  useEffect(() => {
    if (quests.length <= PREVIEW_LIMIT) {
      setShowAllQuests(false);
    }
  }, [quests.length]);

  const visibleQuests = useMemo(() => {
    if (showAllQuests || quests.length <= PREVIEW_LIMIT) {
      return quests;
    }
    return quests.slice(0, PREVIEW_LIMIT);
          ...
          {isLoading ? (
            ...
            <>
                ...
                {visibleQuests.map((quest) => (
                  ...
                  >
                    <button
                      ...
                      onClick={() => {
                        haptics.light();
                        onToggleQuest(quest.id, !quest.completed);
                      }}
                    ...
                    >
                      {quest.completed ? <Check className="h-4 w-4 text-primary" /> : null}
                    </button>
              ...
              {hiddenQuestCount > 0 ? (
                ...
                >
                  {showAllQuests ? "Show less" : `Show ${hiddenQuestCount} more quest${hiddenQuestCount === 1 ? "" : "s"}`}
                </Button>
              ) : null}
            </>
          )}
