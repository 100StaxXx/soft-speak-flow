...
interface SmartEpicWizardProps {
  ...
  /** Clarification answers from the epic clarification flow */
  clarificationAnswers?: ClarificationAnswers;
  /** Context type (e.g., 'exam_preparation', 'fitness_goal') */
  epicContext?: string;
  /** If true, automatically generate suggestions on mount and skip to suggestions step */
...
export function SmartEpicWizard({
  ...
  // Track if we've auto-generated to avoid re-triggering
  ...
  const calculatedTargetDays = useMemo(() => {
    ...
    const dateStr = clarificationAnswers?.exam_date || clarificationAnswers?.target_date;
  ...
  // Auto-generate suggestions when wizard opens with clarification data
  useEffect(() => {
    if (
      open && 
      ...
      // Auto-generate with full context
      generateSuggestions(initialGoal, {
        targetDays: calculatedTargetDays,
        clarificationAnswers,
        epicContext,
      }).then(() => {
        setStep('suggestions');
  ...
  }, [open, autoGenerateSuggestions, initialGoal, selectedTemplate, calculatedTargetDays, clarificationAnswers, epicContext, generateSuggestions, success]);

  // Reset auto-generate flag when wizard closes
  useEffect(() => {
    if (!open) {
  ...
  const handleGenerateSuggestions = useCallback(async () => {
    ...
    tap();
    await generateSuggestions(goalInput, { 
      targetDays,
      clarificationAnswers,
      epicContext,
    });
    setStep('suggestions');
    success();
  }, [goalInput, targetDays, clarificationAnswers, epicContext, generateSuggestions, tap, success]);
  ...
  const handleClose = useCallback(() => {
    ...
    setTargetDays(calculatedTargetDays);
    setEpicTitle('');
    setEpicDescription('');
    setCustomHabits([]);
    setStoryType(null);
