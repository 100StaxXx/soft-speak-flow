...
export const getConnectionErrorFallback = (_mentorName: string): FallbackResponse => {
  return {
    content: `Live guidance is unavailable right now. Pause, bring what is on your mind to God, and choose the next faithful step you already know to take. You can return here when the connection is restored.`,
    isFallback: true
  };
};
...
export const getRateLimitFallback = (_mentorName: string): FallbackResponse => {
  return {
    content: `You've reached today's Guide conversation limit. Use this pause to pray, revisit the guidance you already received, and take one faithful step. You can return tomorrow.`,
    isFallback: true
  };
};
