...
const CompanionPlannerRequestValidationSchema = z.object({
  ...
  plannerContext: z.object({
    ...
    rituals: z.array(z.object({
      ...
      customDays: z.array(z.number()).nullable().optional(),
      customMonthDays: z.array(z.number()).nullable().optional(),
      customPeriod: z.enum(["week", "month"]).nullable().optional(),
...
export interface PlannerRequestValidationIssue {
  path: string;
  code: string;
  message: string;
}

export type CompanionPlannerRequestValidationResult =
  | { success: true }
  | {
    success: false;
    issues: PlannerRequestValidationIssue[];
  };

export const validateCompanionPlannerRequest = (
  request: CompanionPlannerRequest,
): CompanionPlannerRequestValidationResult => {
  const parsed = CompanionPlannerRequestValidationSchema.safeParse(request);
