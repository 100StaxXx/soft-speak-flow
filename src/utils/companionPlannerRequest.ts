...
const sanitizeTaskLikeEntry = (
  ...
  return {
    ...
    flexibility: asNullableString(entry.flexibility) as PlannerContext["tasks"][number]["flexibility"],
    energyType: asNullableString(entry.energyType) as PlannerContext["tasks"][number]["energyType"],
    mustCalendarBlock: asNullableBoolean(entry.mustCalendarBlock),
    deadlineAt: asNullableString(entry.deadlineAt),
...
const scopePlannerTasksToActiveCampaigns = <
  T extends PlannerContext["tasks"][number],
>(
  tasks: T[],
  activeEpicIds: ReadonlySet<string>,
  activeRitualIds: ReadonlySet<string>,
...
export const sanitizePlannerContext = (
  ...
  const reflectionSignals = (context.reflectionSignals ?? [])
    ...
    >[number] => Boolean(entry));
  const activeEpics = context.activeEpics
    .map(sanitizeEpic)
