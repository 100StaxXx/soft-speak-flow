...
import {
  fetchDailyTaskByIdRemote,
  type DailyTask,
} from "@/services/dailyTasksRemote";
...
} from "@/hooks/useTaskCompletionWithInteraction";
import { InteractionLogModal } from "@/components/tasks/InteractionLogModal";
import {
  getCalendarSendSuccessCopy,
...
import {
  JOURNEYS_RESET_TO_TODAY_EVENT,
  JOURNEYS_ROUTE,
} from "@/pages/journeysDateSync";
...
const parseDateKeyAtNoon = (dateKey: string): Date =>
  parseISO(`${dateKey}T12:00:00`);
...
const Journeys = () => {
  ...
  const { favoriteColor: companionFavoriteColor } =
  ...
  const [datePillCenterRequestDateKey, setDatePillCenterRequestDateKey] =
    useState(() => getEffectiveMissionDate());
  const [showPageInfo, setShowPageInfo] = useState(false);
  const [showAddSheet, setShowAddSheet] = useState(false);
  const [isCompanionPlannerPinned, setIsCompanionPlannerPinned] =
    useState(false);
  const [plannerLaunchIntent, setPlannerLaunchIntent] =
    useState<CompanionPlannerLaunchIntent | null>(null);
  const [showMonthView, setShowMonthView] = useState(false);
  const [desktopPlannerMode, setDesktopPlannerMode] =
    useState<DesktopPlannerMode>("week");

  const [prefilledTime, setPrefilledTime] = useState<string | null>(null);
  const [questSheetPrefillDraft, setQuestSheetPrefillDraft] =
    useState<QuestComposerPrefillDraft | null>(null);
  const [questSheetPrefillKey, setQuestSheetPrefillKey] = useState<
    string | null
  >(null);
  const [autoRestoreQuestDraftOnOpen, setAutoRestoreQuestDraftOnOpen] =
    useState(false);
  ...
  const [pathfinderResumeDraft, setPathfinderResumeDraft] =
    useState<CampaignBuilderDraftSnapshot | null>(null);
  const [pathfinderResumeDraftKey, setPathfinderResumeDraftKey] = useState<
    string | null
  >(null);
  const [showCreatedAnimation, setShowCreatedAnimation] = useState(false);
  const [createdCampaignData, setCreatedCampaignData] =
    useState<CreatedCampaignData | null>(null);
  ...
  const scheduledTimeUpdateQueueRef = useRef<Map<string, Promise<void>>>(
    new Map(),
  );
  ...
  const isJourneysRouteActive =
    isTabActive && location.pathname === JOURNEYS_ROUTE;
  ...
  const [editingTask, setEditingTask] = useState<EditableQuestTask | null>(
    null,
  );
  ...
  const handleAddQuestSheetOpenChange = useCallback(
    (nextOpen: boolean) => {
  ...
  const openAddQuestSheet = useCallback(
    (options?: {
      ...
      setPrefilledTime(
        options?.time ?? options?.prefillDraft?.scheduledTime ?? null,
      );
    ...
    (intent?: CompanionPlannerLaunchIntent | null) => {
      if (intent) {
        if (intent.target === "campaign_builder") {
          openCampaignBuilder(
            resolveCampaignBuilderInitialGoal(intent.message, undefined),
          );
          return;
        }
  ...
  const {
    ...
    onFinalResult: (transcript) => {
      ...
      const prefillDraft =
        buildVoiceQuestPrefillFromTranscript(cleanedTranscript);
  ...
  const openCampaignBuilderFromAssistant = useCallback(
    (message: string) => {
      const parsed = parseNaturalLanguage(message);
      const initialGoal = resolveCampaignBuilderInitialGoal(
        message,
        parsed.text,
      );
  ...
  const handlePathfinderOpenChange = useCallback(
    (nextOpen: boolean) => {
  ...
  const resetSelectedDateToToday = useCallback(
    (options?: { deferIfAddSheetOpen?: boolean }) => {
      ...
      setSelectedDate((current) =>
        isSameDay(current, effectiveTodayDate) ? current : effectiveTodayDate,
      );
    },
  ...
  useEffect(() => {
    const previousEffectiveTodayDateKey =
      previousEffectiveTodayDateKeyRef.current;
    previousEffectiveTodayDateKeyRef.current = effectiveTodayDateKey;

    if (
      previousEffectiveTodayDateKey === null ||
      previousEffectiveTodayDateKey === effectiveTodayDateKey
    ) {
      return;
    }
    ...
    if (
      !isJourneysRouteActive ||
      hasUserDateInteractionRef.current ||
      notificationTaskDatePinnedRef.current
    ) {
      return;
    }
  ...
  useLayoutEffect(() => {
    const shouldResetForRouteEntry =
      ...
      (!previousIsJourneysRouteActiveRef.current ||
        previousJourneysLocationSignatureRef.current !==
          journeysLocationSignature);
  ...
  useLayoutEffect(() => {
    if (
      ...
      showAddSheet ||
      showPathfinder ||
      !pendingSelectedDateResetRef.current
    )
      return;
    resetSelectedDateToToday();
  }, [
  ...
  useEffect(() => {
    ...
    const setupListener = async () => {
      const appStateHandle = await CapacitorApp.addListener(
        "appStateChange",
        ({ isActive }) => {
          if (!isActive) return;
          requestAppOpenDateReset();
      ...
      const resumeHandle = await CapacitorApp.addListener(
        "resume",
        requestAppOpenDateReset,
      );
  ...
  const isCompanionPlannerBlocked =
    showAddSheet ||
    showMonthView ||
    showPageInfo ||
    showPathfinder ||
  ...
  const showCompanionPlanner =
    isCompanionPlannerPinned && !isCompanionPlannerBlocked;
  ...
  useEffect(() => {
    ...
    const shouldKeepPlannerOpenForPlanMyDay =
      isTabActive &&
      location.pathname === JOURNEYS_ROUTE &&
      tutorialStep === "plan_my_day";
  ...
  const { tasks: weekCalendarTasks } = useCalendarTasks(selectedDate, "week", {
    enabled: isTabActive,
  });
  ...
  useEffect(() => {
    ...
    const nextCreateQuestRequest =
      routeState?.journeysCreateQuestRequest ?? null;
  ...
  const { surfaceAllEpicHabits, unsurfacedEpicHabitsCount } =
    useHabitSurfacing(selectedDate);

  // Recurring task spawner - auto-spawn tasks with is_recurring = true
  const { pendingRecurringCount, spawnRecurringTasks } =
    useRecurringTaskSpawner(selectedDate);
  ...
  const onboardingData =
    (profile?.onboarding_data as Record<string, unknown> | null) ?? null;
  ...
  useEffect(() => {
    ...
    if (task) {
      ...
      logger.log(
        "[Journeys] Deep link task not found in daily tasks:",
        pendingTaskId,
      );
      deepLinkProcessedRef.current = pendingTaskId;
      clearPendingTask();
  ...
  useEffect(() => {
    ...
    const openNotificationTask = async () => {
      ...
      logger.log(
        "[Journeys] Opening notification-linked task:",
        requestedTaskId,
      );
  ...
  const handleSendTaskToCalendar = useCallback(
    async (taskId: string, requestedTarget?: CalendarSendTarget | null) => {
      ...
      const resolveSendTarget = (): CalendarSendTarget | null => {
        if (
          requestedTarget &&
          isCalendarSendTargetAvailable(requestedTarget, calendarConnections)
        ) {
        ...
        const option = Number.isInteger(pickedIndex)
          ? options[pickedIndex]
          : undefined;
      ...
      const showSuccess = (results: Awaited<ReturnType<typeof attempt>>) => {
        if (results.length === 1) {
          const copy = getCalendarSendSuccessCopy(
            results[0],
        ...
        const destinations = results
          .map(
            (result) =>
              `${result.providerLabel} ${result.destinationKind === "todo" ? "To Do" : "Calendar"} -> ${result.destinationName}`,
          )
          .join("; ");
      ...
      try {
        ...
        let message =
          error instanceof Error
            ? error.message
            : "Failed to send quest to calendar";
        if (message.includes("MULTI_DAY_MONTHLY_UNSUPPORTED")) {
          toast.error(
            "Calendar send doesn't support multi-day monthly recurrence yet.",
          );
          return;
        }
        ...
        if (message.includes("CALENDAR_DEFAULT_REQUIRED")) {
          toast.error(
            "Choose a default calendar provider before sending. Opening Preferences...",
          );
          handleOpenCalendarPreferences();
          return;
        }
        ...
        for (let attemptIndex = 0; attemptIndex < 2; attemptIndex += 1) {
          ...
          if (message.includes("TASK_DATE_REQUIRED") && !taskDateOverride) {
            ...
            if (!pickedDate || !DATE_INPUT_REGEX.test(pickedDate)) {
              toast.error(
                "Calendar send cancelled. Please choose a valid YYYY-MM-DD date.",
              );
              return;
            }
          ...
          if (
            ...
            const pickedTime = window.prompt(
              "Choose a time to send this quest (HH:mm)",
              "09:00",
            );
            if (!pickedTime || !TIME_24H_REGEX.test(pickedTime)) {
              toast.error(
                "Calendar send cancelled. Please choose a valid HH:mm time.",
              );
              return;
            }
        ...
        if (message.includes("TASK_DATE_REQUIRED")) {
          toast.error(
            "Please assign a date before sending this quest to calendar.",
          );
          return;
        }
        ...
        if (
          ...
          toast.error(
            "Please assign a time before sending this quest to calendar.",
          );
          return;
        }
  ...
  const handleAddQuest = useCallback(
    async (data: AddQuestData) => {
      ...
      if (SEND_TO_CALENDAR_ENABLED && data.sendToCalendar && createdTask?.id) {
        ...
        void handleSendTaskToCalendar(
          createdTask.id,
          data.sendToCalendarTarget,
        ).finally(() => {
  ...
  const handleTimelineScheduledTimeUpdate = useCallback(
    (taskId: string, newTime: string) => {
      const previousTaskUpdate =
        scheduledTimeUpdateQueueRef.current.get(taskId) ?? Promise.resolve();
      const nextTaskUpdate = previousTaskUpdate
        ...
        .catch((error) => {
          logger.warn(
            "Failed to update quest scheduled time from timeline drag",
            {
              taskId,
              error: error instanceof Error ? error.message : String(error),
            },
        ...
        .finally(() => {
          if (
            scheduledTimeUpdateQueueRef.current.get(taskId) === nextTaskUpdate
          ) {
  ...
  const handleDeleteQuest = useCallback(
    async (taskId: string, isAIGenerated?: boolean) => {
  ...
  const createRestorableTaskData = useCallback(
    (task: DailyTask) => ({
      task_text: task.task_text,
  ...
  const handleDeleteQuestFromWeekPlanner = useCallback(
  ...
  const handleMoveQuestToNextDayFromWeekPlanner = useCallback(
    async (task: DailyTask) => {
      if (!task.task_date) return;
  ...
  const handleToggleInboxQuest = useCallback(
  ...
  const handleDeleteInboxQuest = useCallback(
    async (taskId: string) => {
  ...
  const handleDeleteEditingQuest = useCallback(
    async (taskId: string) => {
  ...
  const handleSwipeDeleteQuest = useCallback(
    async (taskId: string) => {
  ...
  const handleSwipeMoveToNextDay = useCallback(
    async (taskId: string) => {
  ...
  const handleCreateCampaign = useCallback(
    async (data: Parameters<typeof createEpic>[0]) => {
      try {
  ...
  const handleAnimationComplete = useCallback(() => {
    ...
    window.dispatchEvent(
      new CustomEvent(CAMPAIGN_CREATED_ANIMATION_COMPLETE_EVENT),
    );
  }, []);
      ...
      >
        <div className="mx-auto w-full max-w-[1360px]">
              ...
              {isDesktopLayout && desktopPlannerMode === "week" ? (
                <DesktopWeekPlanner
                  ...
                  timedTaskDurationFallbackMinutes={
                    macTimedTaskDurationFallbackMinutes
                  }
          ...
          <JourneysCompanionPlannerModal
            ...
            presentation={
              isDesktopLayout || isMacHostedIOSApp ? "dialog" : "drawer"
            }
          ...
          <EditQuestDialog
            ...
            isSaving={isUpdating}
            onSendToCalendar={
              SEND_TO_CALENDAR_ENABLED ? handleSendTaskToCalendar : undefined
            }
            hasCalendarLink={
              editingTask ? hasLinkedEvent(editingTask.id) : false
            }
