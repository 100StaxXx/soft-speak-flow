...
describe("useQuestCalendarSync", () => {
  ...
  it("full sync pull for Google refreshes linked events and planner availability", async () => {
    const { result } = renderHook(() => useQuestCalendarSync(), {
      wrapper: createWrapper(),
    });

    await act(async () => {
      await result.current.syncProviderPull.mutateAsync({ provider: "google" });
    });

    expect(mocks.functionsInvokeMock).toHaveBeenNthCalledWith(1, "google-calendar-events", {
      body: { action: "syncLinkedChanges" },
    });
    expect(mocks.functionsInvokeMock).toHaveBeenNthCalledWith(2, "google-calendar-events", {
      body: { action: "syncPlannerWindow" },
    });
  });
  ...
  it("automatically publishes new quests to the default full-sync provider", async () => {
    mocks.useCalendarIntegrationsMock.mockReturnValue({
      connections: [
        {
          id: "conn-google-auto",
    ...
    mocks.dailyTaskSingleMock
      .mockResolvedValueOnce({
        data: {
          id: "task-auto-1",
          task_text: "Automatically synced quest",
    ...
    await act(async () => {
      await result.current.syncTaskUpdate.mutateAsync({ taskId: "task-auto-1" });
    });
