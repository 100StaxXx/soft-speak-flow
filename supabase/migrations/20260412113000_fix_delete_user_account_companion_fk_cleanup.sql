-- Harden delete_user_account for companion-dependent cleanup ordering and
-- clearer diagnostics when a final profile delete is still blocked.
...
BEGIN
  ...
  -- Companion-linked tables can still block the final profile delete when
  -- their inter-table dependencies are only partially removed by the generic
  -- user_id sweep. Clear them explicitly first.
  IF to_regclass('public.companion_evolutions') IS NOT NULL THEN
    BEGIN
      EXECUTE
        'DELETE FROM public.companion_evolutions
         WHERE companion_id = ANY($1)'
      USING companion_ids;
    EXCEPTION WHEN undefined_column OR undefined_table THEN
      NULL;
    END;
    ...
    BEGIN
      EXECUTE
        'DELETE FROM public.companion_evolution_cards
         WHERE companion_id = ANY($1) OR user_id = $2'
      USING companion_ids, p_user_id;
    EXCEPTION WHEN undefined_column OR undefined_table THEN
      NULL;
    END;
  END IF;
