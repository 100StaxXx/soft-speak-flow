-- Restore delete_user_account to relational cleanup only.
-- Storage objects must be removed through the Storage API, not direct SQL.
-- Auth user deletion is handled in the delete-user edge function after storage
-- cleanup succeeds, which keeps account deletion compatible with Supabase's
-- storage protections and avoids multi-attempt partial success flows.
