...
WHERE image_url IS NOT NULL
  AND (
    jsonb_typeof(image_urls) IS DISTINCT FROM 'array'
    OR jsonb_array_length(image_urls) = 0
  );

ALTER TABLE public.companion_cosmiq_title_cards
DROP CONSTRAINT IF EXISTS companion_cosmiq_title_cards_image_urls_array;

ALTER TABLE public.companion_cosmiq_title_cards
ADD CONSTRAINT companion_cosmiq_title_cards_image_urls_array
...
DROP FUNCTION IF EXISTS public.complete_cosmiq_title_card_generation(TEXT, TEXT, TEXT, TEXT);
...
REVOKE ALL ON FUNCTION public.complete_cosmiq_title_card_generation(TEXT, TEXT, TEXT, JSONB, TEXT)
FROM PUBLIC, anon, authenticated;
...
GRANT EXECUTE ON FUNCTION public.complete_cosmiq_title_card_generation(TEXT, TEXT, TEXT, JSONB, TEXT)
TO service_role;
