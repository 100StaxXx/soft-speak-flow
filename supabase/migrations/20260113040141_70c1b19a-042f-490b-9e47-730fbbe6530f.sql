-- Fix delete_user_account function to only reference existing tables
...
BEGIN
  ...
  -- Guild/Community
  DELETE FROM public.guild_artifact_unlocks WHERE user_id = p_user_id;
  ...
  DELETE FROM public.guild_raid_scores WHERE user_id = p_user_id;
  DELETE FROM public.guild_ritual_attendance WHERE user_id = p_user_id;
  DELETE FROM public.guild_shouts WHERE user_id = p_user_id;
  ...
  DELETE FROM public.shout_push_log WHERE user_id = p_user_id;
  ...
  DELETE FROM public.playlist_items WHERE playlist_id IN (SELECT id FROM public.playlists WHERE user_id = p_user_id);
