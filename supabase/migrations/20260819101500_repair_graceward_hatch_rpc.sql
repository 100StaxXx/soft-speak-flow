-- Recovered verbatim statements from the applied shared-production migration history.
-- Source recovery only; do not reapply to production.
-- Repair production drift in the hatch RPC. A later remote definition tried to
-- create companion_animation_jobs without the table's required stage and prompt
-- columns, which caused the whole hatch transaction to roll back. Hatch claims
-- only the prepared stage-one evolution; animation prewarm/processing remains a
-- separate, best-effort pipeline.
