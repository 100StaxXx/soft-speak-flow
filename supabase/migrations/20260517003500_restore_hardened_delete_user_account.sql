-- Restore the hardened account deletion RPC after later migrations regressed
-- the cleanup ordering. Storage metadata cleanup must never block account
-- deletion; the Edge Function handles auth deletion after this RPC succeeds.
...
DECLARE
  ...
  table_record record;
  fk_record record;
  companion_ids uuid[] := ARRAY[]::uuid[];
  fk_cleanup_pass integer;
  blocking_constraint text;
  blocking_message text;
  blocking_detail text;
  blocking_hint text;
  blocking_sqlstate text;
BEGIN
  ...
  -- Clear any companion-owned rows before the generic user_id sweep attempts
  -- to delete user_companion. This covers older and newly added companion
  -- tables without relying on migration-specific ordering.
  FOR table_record IN
    ...
    WHERE c.table_schema = 'public'
      ...
      AND c.column_name = 'companion_id'
    ORDER BY c.table_schema, c.table_name
  LOOP
    BEGIN
      EXECUTE format(
        'DELETE FROM %I.%I WHERE companion_id = ANY($1)',
        table_record.table_schema,
        table_record.table_name
      )
      USING companion_ids;
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'delete_user_account: skipping companion_id cleanup for %.% due to %',
        table_record.table_schema,
        table_record.table_name,
        SQLERRM;
    END;
  ...
  FOR fk_cleanup_pass IN 1..2 LOOP
    FOR fk_record IN
      SELECT
        kcu.table_schema,
        kcu.table_name,
    ...
    LOOP
      BEGIN
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        RAISE LOG 'delete_user_account: FK cleanup pass % skipped for %.%(%): %',
          fk_cleanup_pass,
          fk_record.table_schema,
          fk_record.table_name,
          fk_record.column_name,
  ...
  -- SQL-side storage cleanup is a fallback for account deletion only. The
  -- Storage API path already made its best effort, so failures here should not
  -- turn into "account data" deletion failures.
  IF to_regclass('storage.objects') IS NOT NULL THEN
    BEGIN
      EXECUTE 'DELETE FROM storage.objects WHERE owner = $1'
      USING p_user_id;
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'delete_user_account: storage.objects owner cleanup skipped due to %', SQLERRM;
    END;

    BEGIN
      EXECUTE 'DELETE FROM storage.objects WHERE owner_id = $1'
      USING p_user_id::text;
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'delete_user_account: storage.objects owner_id cleanup skipped due to %', SQLERRM;
    END;

    BEGIN
      EXECUTE
        'DELETE FROM storage.objects
         WHERE bucket_id IN (''quest-attachments'', ''mentors-avatars'', ''journey-paths'', ''companion-images'', ''companion-animation-videos'')
           AND split_part(name, ''/'', 1) = $1'
      USING p_user_id::text;
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'delete_user_account: storage.objects prefix cleanup skipped due to %', SQLERRM;
    END;

    BEGIN
      EXECUTE
        'DELETE FROM storage.objects
         WHERE (bucket_id = ''journey-paths'' AND name LIKE $1)
            OR (bucket_id = ''evolution-cards'' AND name LIKE $2)'
      USING
        'campaign-welcome/welcome-' || p_user_id::text || '-%',
        'postcards/' || p_user_id::text || '/%';
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG 'delete_user_account: storage.objects legacy cleanup skipped due to %', SQLERRM;
    END;
  END IF;
  ...
  EXCEPTION WHEN OTHERS THEN
    GET STACKED DIAGNOSTICS
      blocking_constraint = CONSTRAINT_NAME,
      blocking_message = MESSAGE_TEXT,
      blocking_detail = PG_EXCEPTION_DETAIL,
      blocking_hint = PG_EXCEPTION_HINT,
      blocking_sqlstate = RETURNED_SQLSTATE;

    IF COALESCE(blocking_constraint, '') <> '' THEN
      RAISE EXCEPTION USING
        ERRCODE = COALESCE(blocking_sqlstate, '23503'),
        MESSAGE = format(
          'delete_user_account profile delete blocked by constraint %s',
          blocking_constraint
        ),
        DETAIL = COALESCE(blocking_detail, blocking_message, 'No additional detail available'),
        HINT = blocking_hint;
    END IF;
