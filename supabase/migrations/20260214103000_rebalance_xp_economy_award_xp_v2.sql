-- XP economy rebalance: normalize ledger and move all XP writes to award_xp_v2.

-- Normalize legacy column names first when needed.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    EXECUTE 'ALTER TABLE public.xp_events RENAME COLUMN metadata TO event_metadata';
  END IF;
  ...
  IF EXISTS (
    ...
    EXECUTE 'ALTER TABLE public.xp_events RENAME COLUMN xp_amount TO xp_earned';
  END IF;
END $$;
...
BEGIN
  IF EXISTS (
    ...
    EXECUTE 'UPDATE public.xp_events SET xp_earned = COALESCE(xp_earned, xp_amount)';
  END IF;
  ...
  IF EXISTS (
    ...
    EXECUTE 'UPDATE public.xp_events SET event_metadata = COALESCE(event_metadata, metadata)';
  END IF;
END $$;
...
-- Remove old positive-only XP constraints so undo/deductions are allowed.
DO $$
DECLARE
  xp_constraint RECORD;
BEGIN
  FOR xp_constraint IN
    SELECT conname, pg_get_constraintdef(oid) AS constraint_def
    FROM pg_constraint
    WHERE conrelid = 'public.xp_events'::regclass
      AND contype = 'c'
  LOOP
    IF xp_constraint.constraint_def ILIKE '%xp_amount > 0%'
      OR xp_constraint.constraint_def ILIKE '%xp_amount <= 100%'
      OR xp_constraint.constraint_def ILIKE '%xp_earned > 0%'
      OR xp_constraint.constraint_def ILIKE '%xp_earned <= 100%'
    THEN
      EXECUTE format(
        'ALTER TABLE public.xp_events DROP CONSTRAINT IF EXISTS %I',
        xp_constraint.conname
      );
    END IF;
  END LOOP;
END $$;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_xp_events_user_idempotency_key
  ON public.xp_events(user_id, idempotency_key)
  WHERE idempotency_key IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_xp_events_user_event_type_created_at
  ON public.xp_events(user_id, event_type, created_at DESC);

CREATE INDEX IF NOT EXISTS idx_xp_events_task_id
  ON public.xp_events ((event_metadata->>'task_id'))
  WHERE event_type IN ('task_complete', 'task_undo');

CREATE INDEX IF NOT EXISTS idx_xp_events_pep_talk_id
  ON public.xp_events ((event_metadata->>'pep_talk_id'))
  WHERE event_type = 'pep_talk_listen';
