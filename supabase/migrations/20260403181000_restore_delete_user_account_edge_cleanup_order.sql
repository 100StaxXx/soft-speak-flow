-- Restore delete_user_account to only remove relational app data.
-- Auth user deletion stays in the delete-user edge function so storage cleanup
-- can run before auth.admin.deleteUser. Supabase auth deletion can fail while
-- the user still owns storage objects, so deleting auth.users inside this RPC
-- breaks the previously working flow.
