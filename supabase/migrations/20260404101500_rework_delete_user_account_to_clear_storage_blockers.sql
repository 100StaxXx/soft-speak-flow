-- Rework account deletion so the database routine clears storage ownership
-- blockers and deletes auth.users in the same transaction. This keeps the
-- edge function fast and avoids repeated user attempts caused by long,
-- partially completed storage sweeps inside the request path.
      ...
      EXECUTE
        'DELETE FROM storage.objects
         WHERE owner = $1
            OR owner_id = $2
            OR (
              bucket_id IN (''quest-attachments'', ''mentors-avatars'', ''journey-paths'')
