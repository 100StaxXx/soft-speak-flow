...
Deno.test("companion chat thread backfill migration preserves archive history invariants", async () => {
  const source = await Deno.readTextFile(
    new URL("../../migrations/20260419150000_backfill_companion_chat_threads.sql", import.meta.url),
  );
  ...
  assert(
    source.includes("DISTINCT ON (chats.session_id, chats.user_id, chats.companion_id, chats.surface)"),
    "Expected the backfill to derive one title and preview row per chat session",
  );
  assert(
    source.includes("PARTITION BY stats.user_id, stats.companion_id, stats.surface"),
    "Expected only one active thread per user, companion, and surface",
  );
  assert(
    source.includes("CASE WHEN ranked.session_rank = 1 THEN NULL ELSE ranked.last_message_at END AS archived_at"),
