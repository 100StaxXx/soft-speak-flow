...
async function outlookApi(
  ...
  if (!resp.ok) {
    ...
    throw new Error(`Outlook Calendar API ${method} ${path} failed (${resp.status}): ${details}`);
  }
...
function isOutlookNotFoundError(error: unknown): boolean {
  ...
  return message.includes("(404)") || message.includes("(410)") || message.includes("erroritemnotfound") || message.includes("not found");
}
