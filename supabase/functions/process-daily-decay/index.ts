...
const DECAY_IMAGE_AUTOGENERATION_ENV = "COMPANION_DECAY_IMAGE_AUTOGENERATION";
...
function shouldAutogenerateDecayImages(): boolean {
  return truthyEnv(DECAY_IMAGE_AUTOGENERATION_ENV);
}

async function maybeInvokeDecayImage(
  functionName:
    | "generate-neglected-companion-image"
    | "generate-dormant-companion-image",
  companionId: string,
): Promise<void> {
  if (!shouldAutogenerateDecayImages()) {
    console.log(
      `[Daily Processing] Skipping ${functionName}; ${DECAY_IMAGE_AUTOGENERATION_ENV} is disabled`,
    );
    return;
  }

  await invokeInternalFunction(functionName, { companionId });
}
