...
const MAX_CANONICAL_REFERENCE_DATA_URL_LENGTH = 1_100_000;
const CANONICAL_REFERENCE_DATA_URL_PATTERN =
  /^data:image\/(?:png|jpe?g|webp);base64,[A-Za-z0-9+/=]+$/;

const normalizeCanonicalReferenceImageDataUrl = (
  value: unknown,
): string | null => {
  ...
  if (
    trimmed.length === 0 ||
    trimmed.length > MAX_CANONICAL_REFERENCE_DATA_URL_LENGTH ||
    !CANONICAL_REFERENCE_DATA_URL_PATTERN.test(trimmed)
  ) {
    return null;
  }
  return trimmed;
...
> = {
  // === GRACEWARD SYMBOLIC COMPANIONS ===
  "Lamb": {
    cat: "mammal",
    baby:
      "small white lamb, soft wool, oversized ears, gentle dark eyes, short steady legs",
    adult:
      "healthy white sheep with dense wool, gentle attentive face, grounded stance",
    notes: "Real sheep anatomy, cloven hooves, wool coat, peaceful expression",
    prohibited:
      "wings, horns on a young lamb, predator features, glowing deity imagery",
    ref: "domestic lamb",
  },
  "Stag": {
    cat: "mammal",
    baby:
      "young red deer with long legs, large ears, bright attentive eyes, small budding antlers",
    adult:
      "graceful red stag with a balanced branching antler rack, long legs, alert posture",
    notes:
      "Antlers are symmetrical branching bone, cloven hooves, graceful deer proportions",
    prohibited: "wings, fantasy horns, predator teeth, extra antlers",
    ref: "red stag",
  },
  "Dove": {
    cat: "bird",
    baby:
      "soft white fledgling dove, round body, gentle dark eyes, small pale beak",
    adult:
      "graceful white dove with folded wings, clean feather layers, peaceful alert posture",
    notes: "Two legs, two wings, compact dove body, small beak",
    prohibited:
      "four legs, talons of a raptor, oversized beak, halo or deity imagery",
    ref: "white dove",
  },
  ...
  "Leviathan": {
    cat: "aquatic",
    baby: "small friendly sea serpent, scales, bright attentive eyes, serpentine body, soft fin frills",
    adult:
      "large noble sea serpent with layered scales, flowing fin frills, luminous gill lines, and a long continuous swimming tail",
    notes:
      "Graceward aquatic guardian: a legless sea-serpent companion with fins, gills, one head, and warm readable expressions",
    prohibited: "legs, feet, wings, fur, humanoid torso, extra heads, deity imagery, hostile world-ending monster imagery",
...
function describeAgingParam(key: string, value: number): string {
  const descriptions: Record<string, (v: number) => string> = {
    eyes: (v) =>
      v < 50 ? `${v}% - barely open, squinting` : "100% - fully open, curious",
    body: (v) =>
      v > 80
        ? `${v}% spherical baby fat`
        : v > 50
        ? `${v}% - round but elongating`
        : v > 20
        ? `${v}% - approaching adult proportions`
        : `${v}% - lean adult form`,
    fur: (v) =>
      v < 50
        ? `${v}% dry - still damp from egg`
        : "100% - fully developed coat",
    limbs: (v) =>
      v < 30
        ? `${v}% - tiny nubs pressed to body`
        : v < 60
        ? `${v}% - short but visible`
        : v < 90
        ? `${v}% - near adult proportions`
        : "100% - fully developed",
    mobility: (v) =>
      v < 10
        ? `${v}% - cannot move, curled`
        : v < 30
        ? `${v}% - can lift head`
        : v < 60
        ? `${v}% - wobbly walking`
    ...
    muscle: (v) =>
      v === 0
        ? "0% - pure baby fat"
        : v < 30
        ? `${v}% - hint of definition`
        : v < 70
        ? `${v}% - athletic build emerging`
        : `${v}% - peak condition`,
    wings: (v) =>
      v === 0
        ? "0% - invisible nubs"
        ...
        ? `${v}% - proficient flight`
        : v > 100
        ? "EXPANSIVE - mature wing presence"
        : "100% - full wingspan",
    element: (v) =>
      v === 0
        ? "0% - no effects"
        ...
        : "100% - fully expressed through natural palette and light",
    cosmic: (v) =>
      v === 0
        ? "0%"
        : v < 30
        ? `${v}% - first hints of grandeur`
        : v < 60
        ? `${v}% - widening sense of scale`
        : v < 90
        ? `${v}% - flourishing maturity`
        : "100% - complete and awe-inspiring",
  };
...
function generateStagePrompt(
  ...
  const templates: Record<StageCategory, () => string> = {
    egg: () =>
      stage === 0
        ? `A bespoke companion egg resting in gentle ${element}-inspired light.\n\nEGG APPEARANCE:\n- Smooth opalescent surface with subtle iridescent shimmer\n- ${color} undertones glowing softly throughout shell\n- Soft cel-shaded shell material with clean animated linework\n- Size of a large ostrich egg\n\nLINEAGE HINTS:\n- Shell shape and markings quietly hint at the future ${spirit} without showing a body or face\n- Hopeful, intriguing, and grounded in natural growth\n\nPRESENTATION:\n- Isolated in soft ${element}-inspired light and restrained natural particles`
        : `The same bespoke companion egg now has luminous cracks spreading across its surface.\n\nCRACKING EGG:\n- Same egg from Stage 0, with clean animated linework and glowing cracks\n- ${color} light emanating from fractures\n- ${element}-inspired light visible through the cracks\n- Shell beginning to fragment but not yet broken\n\nLINEAGE HINTS:\n- Shape and markings reveal more of the future ${spirit} lineage without showing a face or full body`,

    baby: () =>
      `A TINY ${spirit} baby at ${name} stage.\n\n━━━ SIZE ━━━\n- ${
        agingData?.size || "15%"
      } of adult size - very small\n\n━━━ BABY APPEARANCE ━━━\n- Body: ${
        describeAgingParam("body", agingData?.body || 85)
      }\n- Eyes: ${
        describeAgingParam("eyes", agingData?.eyes || 100)
      }\n- Limbs: ${
        describeAgingParam("limbs", agingData?.limbs || 20)
      }\n- Mobility: ${
        describeAgingParam("mobility", agingData?.mobility || 10)
    ...
    adolescent: () =>
      `A young adolescent ${spirit} in ${name} stage.\n\n━━━ SIZE ━━━\n- ${
        agingData?.size || "50%"
      } of adult size\n\n━━━ ADOLESCENT FEATURES ━━━\n- Body: ${
        describeAgingParam("body", agingData?.body || 45)
      }\n- Limbs: ${
        describeAgingParam("limbs", agingData?.limbs || 75)
      }\n- Muscle: ${
        describeAgingParam("muscle", agingData?.muscle || 40)
      }\n- Wings: ${
        describeAgingParam("wings", agingData?.wings || 60)
      }\n- Elemental: ${
        describeAgingParam("element", agingData?.element || 30)
      }\n\n━━━ COLORS ━━━\n- ${color} fully vibrant now\n- Adult color pattern established\n\nEXPRESSION: Growing confidence, eager, adventurous.`,

    adult: () =>
      `A fully mature ${spirit} at ${name} stage.\n\n━━━ SIZE ━━━\n- ${
        agingData?.size || "100%"
      } adult size\n\n━━━ ADULT FEATURES ━━━\n- Perfect species anatomy at full adult scale\n- Muscle: ${
        describeAgingParam("muscle", agingData?.muscle || 80)
      }\n- Wings: ${
        describeAgingParam("wings", agingData?.wings || 100)
      }\n- Elemental: ${
        describeAgingParam("element", agingData?.element || 75)
      }\n\n━━━ VISUAL NATURE ━━━\n- ${element} appears as a restrained natural palette and lighting language\n- Subtle environmental accents support the silhouette without changing anatomy\n\n━━━ COLORS ━━━\n- ${color} at peak beauty\n- Warm natural luminescence beginning\n\nEXPRESSION: Wise, powerful, magnificent.`,

    mythic: () =>
      `A fully grown ${spirit} at the ${name} stage, presented at majestic natural scale.\n\n━━━ PRESENCE ━━━\n- Species anatomy remains recognizable within the same original animated character model\n- Mature coat or feathers gain refined cel-shaded shape and line detail\n- The sense of a flourishing garden becomes more abundant through natural accents and lighting\n- ${element} appears only as a restrained color and lighting language\n\n━━━ COLORS ━━━\n- ${color} appears in elegant natural highlights\n- Warm dawn rim light, clean linework, expressive eyes, and rich visual depth\n\nEXPRESSION: Wise, steady, protective, and peaceful.`,

    cosmic: () =>
      `A breathtaking mature ${spirit} at the ${name} stage, the grandest visual expression of long-practiced consistency.\n\n━━━ SCALE AND PRESENCE ━━━\n- Noble, readable proportions within the exact same original animated character model\n- Believable ${spirit} anatomy translated into an elegant 2D anime/storybook silhouette\n- Flourishing olive branches, garden growth, warm horizon light, and restrained natural detail imply an expansive peaceful world without becoming a scenic rectangular backdrop\n- ${element} remains a sophisticated palette and atmospheric lighting cue\n\n━━━ COLORS ━━━\n- ${color} appears in refined natural accents\n- Golden-hour light, deep greens, clean confident linework, and premium soft cel shading\n\nEXPRESSION: Serene confidence, seasoned strength, and welcome.`,
  };
...
> = {
  "whimsical": {
    lighting: "soft golden sunlight through leaves",
    atmosphere: "joyful garden wonder",
    palette: "warm pastels with natural flower accents",
    expression: "curious, delighted, warm",
  },
  "epic": {
  ...
  "mysterious": {
    lighting: "evening lamplight with misty atmosphere",
    atmosphere: "contemplative, quiet, resilient",
    palette: "deep blues, indigo, silver accents",
    expression: "steady, thoughtful, wise",
  },
  "triumphant": {
...
const ELEMENT_EFFECTS: Record<string, string> = {
  ...
  "Light": "Golden dawn radiance, soft light beams, warm natural highlights",
  "Cosmic":
    "Deep evening indigo, distant lights, quiet night-sky color accents",
};

function getElementOverlay(element: string): string {
  const effect = ELEMENT_EFFECTS[element] ||
    `${element} expressed through restrained natural color, light, and landscape details`;
  return `\n━━━ VISUAL NATURE: ${element.toUpperCase()} ━━━\nEffect: ${effect}\nNOTE: This is a natural art direction around the creature, not a supernatural power and not a change to species anatomy.`;
}
...
╠══════════════════════════════════════════════════════════════════════════════╣
║ ${isBabyStage ? "BABY" : "ADULT"} FEATURES: ${
    (isBabyStage ? anatomy.babyFeatures : anatomy.adultFeatures).substring(
      0,
      60,
    ).padEnd(60)
  }║
...
function getNegativePrompts(stage: number, spiritAnimal: string): string {
  ...
  const stageNegatives = stage >= 2 && stage <= 7
    ...
    : stage >= 8 && stage <= 12
    ? ["baby proportions", "deity imagery", "reality-warping effects"]
    : [];
...
serve(async (req) => {
  ...
  try {
    ...
    const canonicalReferenceImage = normalizeCanonicalReferenceImageDataUrl(
      canonicalReferenceImageDataUrl,
    );
    ...
    const speciesIdentity = resolveCompanionSpeciesIdentity(spiritAnimal);
    ...
    if (normalizedFlowType === "ai_onboarding_egg" && Number(stage) === 0) {
      ...
      let bootstrapSourceType:
        | "canonical_reference_edit"
        | "standalone_generation" = canonicalReferenceImage
          ? "canonical_reference_edit"
          : "standalone_generation";
      let bootstrapReferenceEditFailure: string | null = null;
      const canonicalStarterPrompt = canonicalReferenceImage
        ? [
          starterPrompt,
          "",
          "Canonical Graceward form reference:",
          "- The attached portrait is the permanent identity seed for this companion line",
          "- Preserve its exact species read, expressive eye and face design, line weight, stylized anatomy, natural markings, palette, gentle expression, and premium 2D anime/storybook cel-shaded finish",
          "- Adapt it into the young starter form and the user's chosen palette without changing the creature into a sibling or unrelated redesign",
          "- Remove the reference garden completely and return only the isolated companion over true transparency",
        ].join("\n")
        : starterPrompt;

      const stageOneAttempt = await runJudgedRender({
        mode: "bootstrap",
        basePrompt: canonicalStarterPrompt,
        referenceImageUrl: canonicalReferenceImage,
        previousLevel: 0,
        nextLevel: 1,
        render: async (prompt) => {
          if (canonicalReferenceImage && !bootstrapReferenceEditFailure) {
            try {
              return await editCompanionImage({
                guardedFetch,
                openAIApiKey: OPENAI_API_KEY,
                prompt,
                size: imageSize,
                ...
                userId: user.id,
                referenceImages: [{ imageUrl: canonicalReferenceImage }],
              });
            } catch (referenceEditError) {
              bootstrapSourceType = "standalone_generation";
              bootstrapReferenceEditFailure =
                referenceEditError instanceof Error
                  ? referenceEditError.message.slice(0, 500)
                  : String(referenceEditError).slice(0, 500);
              console.warn(
                "[CompanionImage] Canonical form reference edit failed; falling back to standalone starter generation",
                {
                  userId: user.id,
                  spiritAnimal,
                  error: bootstrapReferenceEditFailure,
                },
              );
            }
          }

          return await generateCompanionImage({
            guardedFetch,
            openAIApiKey: OPENAI_API_KEY,
            prompt,
            size: imageSize,
      ...
      if (!stageOneAttempt.passed) {
        throw new CompanionImageQualityGateError(
          stageOneAttempt.scores.stageMaturity < JUDGE_MINIMUMS.stageMaturity
            ? "The hidden stage-1 companion was not clearly an infant form."
            : "The hidden stage-1 companion did not pass the required identity and anatomy checks.",
          stageOneAttempt.scores.stageMaturity < JUDGE_MINIMUMS.stageMaturity
            ? "COMPANION_IMAGE_INFANT_GATE_FAILED"
            : "COMPANION_IMAGE_STAGE_ONE_QUALITY_FAILED",
          422,
        );
      }
      ...
      const imageLineageMetadata = buildInitialImageLineageMetadata({
        ...
        generationLog: {
          ...
          artBibleVersion: typeof artBibleVersion === "string"
            ? artBibleVersion.slice(0, 80)
            : null,
          hiddenStageOne: {
            sourceType: bootstrapSourceType,
            referenceEditFailure: bootstrapReferenceEditFailure,
            retryCount: stageOneAttempt.retryCount,
      ...
      if (debug === true) {
        responseData.bootstrap = {
          starterPrompt: canonicalStarterPrompt,
          eggPrompt,
    ...
    // ========================================================================
    console.info(
      "[CompanionImage] Using legacy compatibility render pipeline",
      {
              ...
              parameters: {
                ...
                properties: {
                  ...
                  artStyle: {
                    type: "string",
                    description:
                      "Rendering style description (e.g., clean 2D anime/storybook linework, soft cel shading)",
                  },
                  distinctiveFeatures: {
                    type: "string",
                    description:
    ...
    const spiritLockPromptAddendum = [
    ...
    if (stage === 0) {
      fullPrompt = `${buildAiEggPrompt(visualIdentityProfile)}${spiritLockPromptAddendum}`;
    } else if (stage === 1) {
      fullPrompt = `${
        buildAiEvolutionPrompt({
          profile: visualIdentityProfile,
      ...
      const categoryRules = stage <= 7
        ...
        : "━━━ MATURE STAGE RULES ━━━\n- Preserve believable natural anatomy\n- Increase dignity through posture, light, and a more expansive flourishing environment\n- Keep symbolic visual treatment restrained";

      fullPrompt =
        `GRACEWARD SYMBOLIC ANIMAL COMPANION - Text-to-image with Visual Reference
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
...
━━━ RENDERING STYLE ━━━
Original premium 2D anime/storybook creature-companion art, appealing and dignified, with expressive warm eyes.
MUST use the exact color palette specified above.
Match the art style: ${extractedMetadata.artStyle}
Clean confident linework, rich controlled colors, soft cel shading, and readable highlights.
Not photorealistic, not 3D, not chibi, and not reminiscent of any specific existing franchise or character.`;
    } else {
      // Text-to-image (egg stages, cosmic stages 15-20, or when no previous image/metadata)
      fullPrompt =
        `GRACEWARD SYMBOLIC ANIMAL COMPANION - Premium 2D anime/storybook illustration
...
━━━ RENDERING STYLE ━━━
Original premium 2D anime/storybook creature-companion art, appealing and dignified, with expressive warm eyes.
Clean confident linework, rich controlled colors, soft cel shading, and a readable silhouette.
NOT photorealistic, NOT 3D, NOT chibi, and not reminiscent of any specific existing franchise or character.
Use lifted midtones and clear highlights for mobile readability.`;
    }
      ...
      if (!aiResponse.ok) {
        ...
        if (aiResponse.status === 429) {
          return timedResponse(
            new Response(
              JSON.stringify({
                error: "AI service busy. Try again.",
                code: "RATE_LIMITED",
              }),
              {
                headers: { ...corsHeaders, "Content-Type": "application/json" },
...
Score each aspect from 0-100 and list any issues.`;
        ...
        if (qualityResponse.ok) {
          ...
          if (toolCall?.function?.arguments) {
            ...
            qualityScore = {
              ...
              shouldRetry: scores.overallQuality < 60 ||
    ...
    if (!imageUrl) {
      ...
      throw new Error(
        `No image was generated after all attempts (model=google/gemini-2.5-flash-image-preview, image_size=${imageSize}, attempts=${
          currentAttempt + 1
