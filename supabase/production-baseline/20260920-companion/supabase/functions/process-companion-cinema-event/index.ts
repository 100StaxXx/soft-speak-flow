...
import { prepareHatchSceneEndpoint, HATCH_SCENE_VERSION, HATCH_VIDEO_CONTINUITY } from "../_shared/companionHatchScene.ts";
import {
...
const renderPortrait = async ({
  ...
  // The canonical cutout remains unchanged for portraits, cards and lineage.
  // A hatch video must end on a scene, otherwise end-frame conditioning erases
  // the egg's background even when the motion prompt asks to preserve it.
  let videoEndPath = portraitPath;
  const isHatch = event.boundary_level === 1;
  if (isHatch) {
    await renewEventLease(supabase, event.id, leaseToken);
    const scene = await prepareHatchSceneEndpoint({
      guardedFetch,
      openAIApiKey,
      model: Deno.env.get("COSMIQ_CINEMA_IMAGE_MODEL")?.trim() || "gpt-image-1-mini",
      size: IMAGE_SIZE,
      userId: event.user_id,
      startImageUrl: externalPreviousImageUrl,
      companionImageUrl: best.imageDataUrl,
    });
    await renewEventLease(supabase, event.id, leaseToken);
    videoEndPath = await uploadPrivateAsset({
      supabase,
      userId: event.user_id,
      eventId: event.id,
      fileName: "hatch-scene-endpoint.png",
      bytes: parseDataUrl(scene.imageDataUrl),
      contentType: "image/png",
    });
  }
  ...
  const renderRows = [1, 2].map((candidateIndex) => ({
    ...
    prompt: buildCompanionCinemaVideoPrompt({
      ...
      scenePlan,
      candidateIndex,
    }) + (isHatch ? ` ${HATCH_VIDEO_CONTINUITY}` : ""),
    prompt_version: isHatch ? `${COMPANION_CINEMA_PROMPT_VERSION}-${HATCH_SCENE_VERSION}` : COMPANION_CINEMA_PROMPT_VERSION,
    start_image_url: previousImageUrl,
    end_image_bucket: COMPANION_CINEMA_PRIVATE_BUCKET,
    end_image_path: videoEndPath,
