...
On September 20, the live `process-companion-cinema-event` worker and its two missing cinema helpers were imported with explicit user approval. All of its already-present shared dependencies matched this closure byte-for-byte. The only worker behavior change is a video-only opaque hatch endpoint retaining the opening scene; canonical cutouts, later evolution endpoints, product guards, and cleanup logic remain unchanged. The new root entry point starts the imported handler explicitly. This addition is prepared locally, not deployed.
